...
function SearchableConvenioDropdown({ value, onChange, tipoConvenio }) {
  const [isOpen, setIsOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [convenios, setConvenios] = useState([]);
  const dropdownRef = useRef(null);

  // Leer convenios desde Supabase (se actualiza cuando cambia el tipo)
  useEffect(() => {
    const fetchConvenios = async () => {
      const tipoDb = tipoConvenio === "Público" ? "publico" : "privado";
      const { data } = await supabase
        .from("convenios")
        .select("nombre")
        .eq("tipo", tipoDb)
        .order("nombre");
      if (data) setConvenios(data.map((c) => c.nombre));
    };
    if (tipoConvenio) fetchConvenios();
  }, [tipoConvenio]);

  const filtered = search
    ? convenios.filter((c) => c.toLowerCase().includes(search.toLowerCase()))
    : convenios;
    ...
    <div ref={dropdownRef} style={{ position: "relative" }}>
        ...
        {value ? (
          ...
          >
            ...
            <span
              ...
              style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", cursor: "pointer", color: COLORS.textLight, fontSize: 16, padding: "0 4px" }}
            >✕</span>
          </div>
        ) : (
          <input
      ...
      {isOpen && !value && (
        <div style={{
          ...
          {filtered.length === 0 ? (
            <div style={{ padding: 16, textAlign: "center", color: COLORS.textLight, fontSize: 13 }}>No se encontraron convenios</div>
          ) : (
            filtered.map((c) => (
              <div
                key={c}
              ...
              >
                {search ? (() => {
                  const idx = c.toLowerCase().indexOf(search.toLowerCase());
                  if (idx < 0) return c;
                  return <>{c.substring(0, idx)}<span style={{ background: "#fef08a", borderRadius: 2, padding: "0 1px" }}>{c.substring(idx, idx + search.length)}</span>{c.substring(idx + search.length)}</>;
                })() : c}
              </div>
            ))
          )}
...
// ─── Campos de Nómina (Sucursal + Convenio) — Reutilizable ───
function NominaFields({ form, update, labelStyle, inputStyle }) {
  return (
    <div style={{
      background: `linear-gradient(135deg, ${COLORS.primaryLight} 0%, ${COLORS.blueBg} 100%)`,
      border: `1.5px solid ${COLORS.primary}40`,
      borderRadius: 10, padding: 14, marginTop: 2,
    }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: COLORS.primary, textTransform: "uppercase", letterSpacing: 0.5, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}>
        🏢 Datos de Nómina
      </p>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        {form.tipo_convenio && (
          <div>
            ...
            <SearchableConvenioDropdown
              value={form.convenio || ""}
              onChange={(v) => update("convenio", v)}
              tipoConvenio={form.tipo_convenio}
            />
          </div>
        )}
      </div>
...
function DashboardAdmin() {
    ...
    const fetchAll = async () => {
      ...
      try {
        await fetchWithRetry(async () => {
          ...
          const years = new Set();
          (cRes.data || []).forEach((c) => {
            if (c.anio_registro) years.add(c.anio_registro);
          });
  ...
  // ── Filtrar datos según modo (usando mes_registro/anio_registro para consistencia con ResumenNomina y ResumenMotos) ──
  const dispersados = useMemo(() => {
    const disp = allClients.filter((c) => c.estatus === "Dispersión");
    if (modo === "mensual") {
      return disp.filter((c) => c.mes_registro === mes && c.anio_registro === anio);
    }
    if (modo === "acumulado") {
      return disp.filter((c) => c.anio_registro === anio);
    }
  ...
  const monthlyData = useMemo(() => {
    ...
    const meses = {};
    disp.forEach((c) => {
      const m = c.mes_registro;
      const y = c.anio_registro;
      if (!m || !y) return;
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: COLORS.dark, margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 10, background: `linear-gradient(135deg, ${COLORS.primary}20, ${COLORS.primary}08)` }}>📈</span>
              Ingresos por mes {modo === "mensual" ? "" : `— ${modo === "acumulado" ? anio : "Historial completo"}`}
            </h3>
            {monthlyData.length > 0 && (
              <span style={{ fontSize: 12, fontWeight: 600, color: COLORS.textLight, background: "#F1F5F9", padding: "4px 10px", borderRadius: 20 }}>
                {monthlyData.length} {monthlyData.length === 1 ? "mes" : "meses"}
              </span>
            )}
          </div>
          {monthlyData.length === 0 ? (
            <p style={{ textAlign: "center", color: COLORS.textLight, padding: 30, fontSize: 14 }}>Sin datos para este periodo</p>
          ) : (
            <>
              {/* Líneas guía horizontales */}
              <div style={{ position: "relative", height: isMobile ? 200 : 240, padding: "0 0 40px" }}>
                {[0.25, 0.5, 0.75, 1].map((pct) => (
                  <div key={pct} style={{
                    position: "absolute", left: 0, right: 0,
                    bottom: 40 + ((isMobile ? 160 : 200) * pct),
                    height: 1, background: `${COLORS.border}80`,
                    borderStyle: "dashed", borderWidth: "0 0 1px 0", borderColor: `${COLORS.border}60`,
                  }} />
                ))}
                {/* Barras */}
                <div style={{ display: "flex", alignItems: "flex-end", gap: isMobile ? 3 : 8, height: "100%", position: "relative" }}>
                  {monthlyData.map((item, i) => {
                    const barHeight = isMobile ? 160 : 200;
                    const hIngreso = maxIngreso > 0 ? (item.ingresos / maxIngreso) * barHeight : 0;
                    const hPres = maxIngreso > 0 && item.presupuesto ? (item.presupuesto / maxIngreso) * barHeight : 0;
                    const esMesActual = item.mes === today.getMonth() + 1 && item.anio === today.getFullYear();
                    const cumplioMeta = item.ingresos >= (item.presupuesto || 0) && item.presupuesto > 0;
                    return (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative", height: "100%", justifyContent: "flex-end" }}>
                        {/* Monto completo arriba de la barra */}
                        {item.ingresos > 0 && (
                          <div style={{
                            fontSize: isMobile ? 7 : 10, fontWeight: 800, color: esMesActual ? COLORS.primary : cumplioMeta ? COLORS.green : COLORS.dark,
                            marginBottom: 4, whiteSpace: "nowrap", textAlign: "center",
                            background: esMesActual ? `${COLORS.primary}10` : "transparent",
                            padding: isMobile ? "1px 2px" : "2px 6px", borderRadius: 6,
                            letterSpacing: -0.3,
                          }}>
                            {formatMoney(Math.round(item.ingresos))}
                          </div>
                        )}
                        {/* Barra de presupuesto (fantasma) */}
                        {hPres > 0 && (
                          <div style={{
                        ...
                        {/* Indicador de meta cumplida */}
                        {cumplioMeta && (
                          <span style={{ fontSize: 8, marginTop: 1 }}>✅</span>
                        )}
                        {/* Label del mes */}
                        <span style={{
                          fontSize: isMobile ? 8 : 11, fontWeight: esMesActual ? 800 : 600,
                          color: esMesActual ? COLORS.primary : COLORS.textLight,
                          marginTop: cumplioMeta ? 0 : 5, textAlign: "center",
                          ...(esMesActual ? {
                            background: `${COLORS.primary}12`, padding: "2px 8px", borderRadius: 10,
                          } : {}),
                        }}>
                          {item.label}
                        </span>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 14, height: 14, borderRadius: 5, background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.primaryDark})` }} />
                  <span style={{ fontSize: 11, color: COLORS.text, fontWeight: 600 }}>Ingresos</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 14, height: 14, borderRadius: 5, background: `${COLORS.blue}12`, border: `1.5px dashed ${COLORS.blue}40` }} />
                  <span style={{ fontSize: 11, color: COLORS.text, fontWeight: 600 }}>Presupuesto</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 10 }}>✅</span>
                  <span style={{ fontSize: 11, color: COLORS.text, fontWeight: 600 }}>Meta cumplida</span>
                </div>
              </div>
            </>
          )}
                ...
                {acumuladoData.map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                      {!isMobile && <span style={{ fontSize: 9, fontWeight: 800, color: COLORS.primaryDark, marginBottom: 2, letterSpacing: -0.3 }}>{formatMoney(Math.round(item.acumulado))}</span>}
                      <div style={{
...
function TablaClientes({ perfil }) {
  ...
  const handleAddClient = async (form) => {
    ...
    // Agregar campos de nómina si aplica
    if (form.producto === "Crédito de nómina") {
      clientData.sucursal = form.sucursal || null;
...
function ResumenNomina() {
  ...
  const tableData = useMemo(() => {
    return nominaEjecutivos.map((ej) => {
      ...
      const clientesDisp = clients.filter(
        (c) => c.ejecutivo === ej.nombre && c.estatus === "Dispersión" && c.producto === "Crédito de nómina"
      ).length;
  ...
  const totals = useMemo(() => {
    ...
    const clientesDisp = tableData.reduce((s, e) => s + e.clientesDisp, 0);
    const ticketPromedio = clientesDisp > 0 ? real / clientesDisp : 0;
    const avance = meta > 0 ? (real / meta) * 100 : 0;
    const proyeccion = diasTranscurridos > 0 ? (real / diasTranscurridos) * diasMes : 0;
    const falta = Math.max(meta - real, 0);
    return { meta, real, clientesDisp, ticketPromedio, avance, proyeccion, falta };
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "20px 24px", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1000 }}>
            ...
            <tbody>
              {tableData.map((ej, idx) => {
                ...
                return (
                  <tr key={idx} style={{
                    ...
                    <td style={{ padding: "14px 16px", textAlign: "center" }}>
                      <span style={{ fontSize: 15, fontWeight: 700, color: COLORS.dark }}>{ej.clientesDisp}</span>
                    </td>
                    <td style={{ padding: "14px 16px", fontVariantNumeric: "tabular-nums" }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: COLORS.blue }}>{formatMoney(Math.round(ej.ticketPromedio))}</span>
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{
              ...
              <tr style={{ background: COLORS.dark }}>
                ...
                <td style={{ padding: "14px 16px", textAlign: "center", fontWeight: 700, fontSize: 15, color: "#fff" }}>
                  {totals.clientesDisp}
                </td>
                <td style={{ padding: "14px 16px", fontWeight: 600, fontSize: 12, color: "#9CA3AF", fontVariantNumeric: "tabular-nums" }}>
                  {formatMoney(Math.round(totals.ticketPromedio))}
                </td>
                <td style={{ padding: "14px 16px" }}>
                  <span style={{
                    display: "inline-block", padding: "4px 12px", borderRadius: 20,
        ...
        {/* ═══════════════════════════════════════════════════ */}
        {(() => {
          const nominaClients = clients.filter((c) => c.producto === "Crédito de nómina" && c.convenio);
          const convenioMap = {};

          nominaClients.forEach((c) => {
            if (!convenioMap[c.convenio]) {
              convenioMap[c.convenio] = {
                nombre: c.convenio,
                tipo: c.tipo_convenio || "—",
                total: 0,
                dispersados: 0,
                montoTotal: 0,
                montoDispersado: 0,
            ...
            const entry = convenioMap[c.convenio];
            entry.total++;
            entry.montoTotal += (c.monto || 0);
          ...
          const rankingConvenios = Object.values(convenioMap)
            .sort((a, b) => b.montoDispersado - a.montoDispersado);

          const totalDisp = rankingConvenios.reduce((s, c) => s + c.montoDispersado, 0);
          ...
          return (
            <>
              ...
              <div style={{ display: "flex", gap: 14, marginBottom: 16, flexWrap: "wrap" }}>
                <KPICardNomina icon="📋" label="Convenios activos" value={rankingConvenios.length} color={COLORS.blue} />
                <KPICardNomina icon="👥" label="Clientes por convenio" value={nominaClients.filter(c => c.convenio).length} color={COLORS.dark} />
                <KPICardNomina icon="💰" label="Monto dispersado" value={formatMoney(totalDisp)} color={COLORS.primary} />
                <KPICardNomina icon="✅" label="Tasa de cierre" value={nominaClients.filter(c => c.convenio).length > 0 ? `${((rankingConvenios.reduce((s, c) => s + c.dispersados, 0) / nominaClients.filter(c => c.convenio).length) * 100).toFixed(0)}%` : "0%"} color={COLORS.green} />
              </div>
            ...
            borderRadius: 16,
                  ...
                  <p style={{ fontSize: 15, fontWeight: 600, color: COLORS.dark, margin: "0 0 8px" }}>
                    Aún no hay clientes con convenio asignado
                  </p>
                  <p style={{ fontSize: 13, color: COLORS.textLight, margin: 0, maxWidth: 420, marginLeft: "auto", marginRight: "auto" }}>
                    Los nuevos clientes capturados por ejecutivos de nómina aparecerán aquí con sus métricas por convenio.
                  </p>
                </div>
              ) : (
              <div style={{
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
                  ...
                  <tbody>
                    {rankingConvenios.map((conv, idx) => {
                      const tasaCierre = conv.total > 0 ? (conv.dispersados / conv.total) * 100 : 0;
                      const pctTotal = totalDisp > 0 ? (conv.montoDispersado / totalDisp) * 100 : 0;
                      ...
                      return (
                        <tr key={conv.nombre} style={{
                          ...
                          <td style={{ padding: "12px 14px", fontSize: 13, fontWeight: 600, color: COLORS.text, maxWidth: 280 }}>
                            <p style={{ margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={conv.nombre}>{conv.nombre}</p>
                          </td>
                          <td style={{ padding: "12px 14px" }}>
                            <span style={{
                              display: "inline-block", padding: "3px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700,
                              background: conv.tipo === "Público" ? COLORS.blueBg : conv.tipo === "Privado" ? COLORS.purpleBg : "#f3f4f6",
                              color: conv.tipo === "Público" ? COLORS.blue : conv.tipo === "Privado" ? COLORS.purple : COLORS.textLight,
                            }}>
                              {conv.tipo}
                            </span>
                          </td>
                          ...
                          <td style={{ padding: "12px 14px", fontSize: 14, fontWeight: 700, color: COLORS.primary, fontVariantNumeric: "tabular-nums" }}>
                            {formatMoney(conv.montoDispersado)}
                          </td>
                          <td style={{ padding: "12px 14px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <tr style={{ background: "#F8FAFC" }}>
                      ...
                      <td style={{ padding: "12px 14px" }} />
                      <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: 14, color: "#fff", textAlign: "center" }}>{nominaClients.length}</td>
                      <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: 14, color: COLORS.primary, textAlign: "center" }}>{rankingConvenios.reduce((s, c) => s + c.dispersados, 0)}</td>
                      <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: 14, color: COLORS.blueBg, textAlign: "center" }}>{rankingConvenios.reduce((s, c) => s + c.enPipeline, 0)}</td>
                      <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: 14, color: "#FF8A8A", textAlign: "center" }}>{rankingConvenios.reduce((s, c) => s + c.rechazados, 0)}</td>
                      <td style={{ padding: "12px 14px", fontWeight: 800, fontSize: 14, color: COLORS.primary, fontVariantNumeric: "tabular-nums" }}>{formatMoney(totalDisp)}</td>
                      ...
                      <td style={{ padding: "12px 14px" }}>
                        <span style={{
                          ...
                          color: "#fff", background: nominaClients.length > 0 ? (rankingConvenios.reduce((s, c) => s + c.dispersados, 0) / nominaClients.length * 100 >= 50 ? COLORS.green : COLORS.yellow) : COLORS.textLight,
                        }}>
                          {nominaClients.length > 0 ? ((rankingConvenios.reduce((s, c) => s + c.dispersados, 0) / nominaClients.length) * 100).toFixed(0) : 0}%
                        </span>
                      </td>
                    </tr>
                  </tbody>
...
function ResumenMotos() {
  ...
  const tableData = useMemo(() => {
    return motosEjecutivos.map((ej) => {
      const dispClients = clients.filter(
        (c) => c.ejecutivo === ej.nombre && c.estatus === "Dispersión" && c.producto !== "Crédito de nómina"
      );
      const real = dispClients.length;
      const montoTotal = dispClients.reduce((s, c) => s + (c.monto || 0), 0);

      const arrClients = clients.filter(
        (c) => c.ejecutivo === ej.nombre && c.estatus === "Dispersión" && c.producto === "Arrendamiento de motos"
      );
      const arrendamiento = arrClients.length;
  ...
  const totals = useMemo(() => {
    ...
    const montoTotal = tableData.reduce((s, e) => s + e.montoTotal, 0);
    const montoArr = tableData.reduce((s, e) => s + e.montoArrendamiento, 0);
    const montoFin = tableData.reduce((s, e) => s + e.montoFinanciamiento, 0);
                ...
                return (
                  <tr key={idx} style={{
                    ...
                    <td style={{ padding: "14px", textAlign: "right" }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: COLORS.primary, fontVariantNumeric: "tabular-nums" }}>{formatMoney(Math.round(ej.montoTotal))}</span>
                    </td>
                    <td style={{ padding: "14px", textAlign: "right" }}>
            ...
            borderRadius: 16, padding: 20,
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 700, color: COLORS.yellow, margin: 0 }}>{ej.arrendamiento} uds</p>
                    <p style={{ fontSize: 10, fontWeight: 600, color: COLORS.yellow, margin: 0, opacity: 0.8 }}>{formatMoney(Math.round(ej.montoArrendamiento))}</p>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    <p style={{ fontSize: 10, color: COLORS.purple, margin: 0, fontWeight: 600 }}>Financ.</p>
                    <p style={{ fontSize: 14, fontWeight: 700, color: COLORS.purple, margin: 0 }}>{ej.financiamiento} uds</p>
                    <p style={{ fontSize: 10, fontWeight: 600, color: COLORS.purple, margin: 0, opacity: 0.8 }}>{formatMoney(Math.round(ej.montoFinanciamiento))}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 10, color: COLORS.textLight, margin: 0, textTransform: "uppercase" }}>Falta</p>
...
function GestionConvenios() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [convenios, filterTipo, search]);

  const publicos = convenios.filter((c) => c.tipo === "publico").length;
  const privados = convenios.filter((c) => c.tipo === "privado").length;

  const handleSave = async () => {
    if (!formNombre.trim() || !formTipo) { alert("Llena nombre y tipo"); return; }
    if (editingConvenio) {
      await supabase.from("convenios").update({ nombre: formNombre.trim(), tipo: formTipo }).eq("id", editingConvenio.id);
    } else {
      await supabase.from("convenios").insert({ nombre: formNombre.trim(), tipo: formTipo });
    }
  ...
  const openEdit = (conv) => {
    ...
    setFormNombre(conv.nombre);
    setFormTipo(conv.tipo);
  ...
  return (
    <div style={{ padding: isMobile ? "16px 12px" : "28px 32px", maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: COLORS.dark, margin: "0 0 4px" }}>📋 Gestión de Convenios</h1>
        <p style={{ fontSize: 14, color: COLORS.textLight, margin: 0 }}>Administra los convenios disponibles para ejecutivos de nómina</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={filterTipo} onChange={(e) => setFilterTipo(e.target.value)} style={{ ...inputStyle, width: "auto", cursor: "pointer", minWidth: 150 }}>
          <option value="todos">Todos los tipos</option>
          <option value="publico">🏛 Públicos</option>
          <option value="privado">🏢 Privados</option>
        </select>
        <button onClick={openAdd} style={{ padding: "10px 20px", background: COLORS.primary, color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
          + Agregar Convenio
        </button>
      </div>
            ...
            <tbody>
              {filtered.map((conv, idx) => (
                <tr key={conv.id} style={{ borderBottom: `1px solid #f3f4f6` }}>
                  <td style={{ padding: "10px 16px", fontSize: 12, color: COLORS.textLight }}>{idx + 1}</td>
                  <td style={{ padding: "10px 16px", fontSize: 13, fontWeight: 500 }}>{conv.nombre}</td>
                  <td style={{ padding: "10px 16px" }}>
                    <span style={{
                      display: "inline-block", padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700,
                      background: conv.tipo === "publico" ? COLORS.blueBg : COLORS.purpleBg,
                      color: conv.tipo === "publico" ? COLORS.blue : COLORS.purple,
                    }}>
                      {conv.tipo === "publico" ? "🏛 Público" : "🏢 Privado"}
                    </span>
                  </td>
                  <td style={{ padding: "10px 16px", textAlign: "center" }}>
                    <button onClick={() => openEdit(conv)} style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer", marginRight: 6, fontFamily: "inherit" }}>✏️ Editar</button>
                    <button onClick={() => setDeleteConfirm(conv)} style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer", fontFamily: "inherit", color: COLORS.red }}>🗑</button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={4} style={{ padding: 24, textAlign: "center", color: COLORS.textLight }}>No se encontraron convenios</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", maxWidth: 480, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ display: "grid", gap: 14 }}>
              <div>
                ...
                <input style={inputStyle} value={formNombre} onChange={(e) => setFormNombre(e.target.value)} placeholder="Ej: EMPRESA NUEVA S.A. DE C.V." />
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => { setShowAddModal(false); setEditingConvenio(null); }} style={{ flex: 1, padding: "12px", fontSize: 14, fontWeight: 600, color: COLORS.textLight, background: "#F3F4F6", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={handleSave} style={{ flex: 2, padding: "12px", fontSize: 14, fontWeight: 700, color: "#fff", background: COLORS.primary, border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>
                {editingConvenio ? "Guardar cambios" : "Agregar"}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", maxWidth: 420, width: "90%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <h3 style={{ fontSize: 17, fontWeight: 800, color: COLORS.dark, margin: "0 0 8px" }}>¿Eliminar convenio?</h3>
            <p style={{ fontSize: 14, color: COLORS.text, margin: "0 0 20px", lineHeight: 1.5 }}>
              Se eliminará <strong>{deleteConfirm.nombre}</strong>. Esta acción no se puede deshacer.
            </p>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setDeleteConfirm(null)} style={{ flex: 1, padding: "12px", fontSize: 14, fontWeight: 600, color: COLORS.textLight, background: "#F3F4F6", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={() => handleDelete(deleteConfirm.id)} style={{ flex: 1, padding: "12px", fontSize: 14, fontWeight: 700, color: "#fff", background: COLORS.red, border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
...
function AddModal({ onAdd, onClose, ejecutivoTipo }) {
  const [form, setForm] = useState({ nombre: "", producto: "", monto: "", actualizacion: "", sucursal: "", tipo_convenio: "", convenio: "" });

  const update = (k, v) => {
    setForm((p) => {
      const next = { ...p, [k]: v };
      if (k === "producto" && v !== "Crédito de nómina") {
        next.sucursal = "";
        next.tipo_convenio = "";
        next.convenio = "";
      }
...
function PortalEjecutivo({ perfil }) {
  ...
  // ─── Detectar tipo de ejecutivo (nómina vs motos) ───
  const [ejecutivoTipo, setEjecutivoTipo] = useState(null);
  useEffect(() => {
    const fetchTipo = async () => {
      ...
      const { data } = await supabase
        .from("ejecutivos")
        .select("tipo")
        .eq("nombre", ejecutivoNombre)
        .eq("mes", mes)
        .eq("anio", anio)
        .single();
      if (data?.tipo) setEjecutivoTipo(data.tipo);
    };
    fetchTipo();
      ...
      <main style={{
        ...
        {isEjecutivo ? (
          ...
          <>
            ...
            {activeScreen === "convenios" && <GestionConvenios />}
            {activeScreen === "usuarios" && <GestionUsuarios />}
            {activeScreen === "export" && <ExportExcel />}

            {!["dashboard", "clientes", "nomina", "motos", "catalogo", "convenios", "usuarios", "export"].includes(activeScreen) && (
